...
public class Assasin extends Enemy{
    protected static double hpmax = 1000;
    protected static double hp_default = 1000;
    protected int price = 25;


    private final int t_max_Invisble=5*Game.FPS;//thoi gian thuc hien tang hunh
    private final int t_max_count=5*Game.FPS;//thoi gian dem de thuc hien tang hinh
    protected double hp = hpmax;
    protected double velotical = 0.02;
    public int step = (int)(1.0/velotical);
    protected int countInvisible=0;
    public Assasin(double x, double y) {
        super(x, y);
    }
    @Override
    double getHp() {
    ...
    public void drawEnemy(){
        if(isSlowed){//Neu con quai bi lam cham, thuc hien ve mot tang bang duoi chan no
            ...
            Game.gc.fillOval((this.x+0.25)*size,(this.y+0.5)*size,size/2,size/2);
        }
        Game.gc.setFill(Color.web("000000",0.4));
        Game.gc.fillOval((this.x+0.25)*size,(this.y+0.5)*size,size/2,size/2);

        if(invisible){
            Game.gc.drawImage(Main.assasin_tang_hinh[currentDirrection][count%Game.FPS/(Game.FPS/4)],(x+0.125)*size, (y+0.125)*size,size*0.75,size*0.75);
        }
        else{
            Game.gc.drawImage(Main.assasin_images_list[currentDirrection][count%Game.FPS/(Game.FPS/4)],(x+0.125)*size, (y+0.125)*size,size*0.75,size*0.75);
        }
    ...
    public void updateVisiable(){
        if(!invisible) {
            ...
            if (countInvisible >= t_max_Invisble) {
                invisible = false;
                countInvisible = 0;
