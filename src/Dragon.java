...
public class Dragon extends  Enemy{
    public static double hpmax = 2500;
    protected static double hp_default = 2500;//Dung de gan mau tro lai khi mo man moi
    protected int price = 55;

    protected double hp = hpmax;
    protected double velotical = 0.012;
    ...
    public void drawEnemy(){
        ...
        Game.gc.fillOval((this.x+0.25)*size,(this.y+0.5)*size,size/2,size/2);
        //Ve anh con quai
        int i = count%Game.FPS/(Game.FPS/4);
        Game.gc.drawImage(Main.dragon_images_list[currentDirrection][i],(x-0.3)*size, (y -0.95 - (i)*(4-i)*0.005)*size,size*1.6,size*1.6);
        //Ve mau cua con quai

        Game.gc.setFill(Color.RED);
        Game.gc.fillRect(this.x*size,(y -0.95 - (i)*(4-i)*0.005)*size-4, size,4);
        Game.gc.setFill(Color.GREEN);
        Game.gc.fillRect(this.x*size,(y -0.95 - (i)*(4-i)*0.005)*size-4, hp/hpmax*size,4);
    }
    @Override
