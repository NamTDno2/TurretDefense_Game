...
public class Mortar extends Turret {
    ...
    protected double coolDownMax = 2*Game.FPS;
    ...
    private List<BulletMortar> bullets = new ArrayList<>();
    private List<BulletMortar> bullets_collision = new ArrayList<>();
                ...
                if ((dx * dx + dy * dy) <= range * range && (dx*dx + dy*dy)>=range_min*range_min && !enemyTemp.invisible) {
                    ...
                    if (target.contains(enemyTemp)) {
                        ...
                        for (int j = 0, m = enemyManager.enemies.size(); j < m; ++j) {
                            ...
                            if ((dx1 * dx1 + dy1 * dy1) <= range * range && (dx1 * dx1 + dy1 * dy1)>=range_min*range_min &&!enemyTemp1.invisible) {
            ...
            if (coolDown >= coolDownMax) {//Khi cho hoi chieu du lau thi ban dan vao quan dich
                ...
                while (iterator1.hasNext()) {
                    ...
                    if (enemyTemp.getHp() <= 0) {//Neu ke dich het mau, xoa no
                        ...
                        bullets.add(new BulletMortar(x_center,y_center-0.3,enemyTemp,level,this));//Tao ra vien dan nham vao con quai xuat phat tu vi tri cua tru
                    }
                }
