...
public class Cannon extends Turret{
    public static Image base_turret = new Image("/imgbgr/Turret/Cannon/Đế Cannon.png");
    public static Image[] shaft_turret = {
            new Image("/imgbgr/Turret/Cannon/Cannon Level 1.png"),
            new Image("/imgbgr/Turret/Cannon/Cannon Level 2.png"),
            new Image("/imgbgr/Turret/Cannon/Cannon Level 3.png"),
    };

    protected int coolDownMax = Game.FPS;
    //Xem o class TurretIce
    public Image image = image_list[1];//image_list lay tu class Turret
    protected double range = 4.5;//Tam ban cua tru
    ...
    public void drawTurret() {
        try{
            ...
            if(!target.isEmpty()){
                temp = target.get(0);
                angle = Math.atan2(temp.getY()-y,temp.getX()-x)*180/3.14159;
            }
            else{
                angle = 120;
            }
            ...
            if (coolDown >= coolDownMax) {//Khi cho hoi chieu du lau thi ban dan vao quan dich
                ...
                while (iterator1.hasNext()) {
                    ...
                    if (enemyTemp.getHp() <= 0) {//Neu ke dich het mau, xoa no
                        ...
                        bullets.add(new BulletCannon(x_center, y_center, enemyTemp, level));//Tao ra vien dan nham vao con quai xuat phat tu vi tri cua tru
                    }
                }
