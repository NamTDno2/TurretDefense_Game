...
public class Reaper extends  Enemy{
    protected static double hpmax = 1200;
    protected static double hp_default = 1200;
    protected int price = 25;

    protected double hp = hpmax;
    protected double velotical = 0.017;
    public int step = (int)(1.0/velotical);
    public Reaper(double x, double y) {
        super(x, y);
    }
    @Override
    double getHp() {
    ...
    public void drawEnemy(){
        ...
        Game.gc.drawImage(Main.death_images_list[currentDirrection][0],x*size, (y -  0.25 - (i)*(4-i)*0.01625)*size,size,size);
        //Ve mau cua con quai

        Game.gc.setFill(Color.RED);
        Game.gc.fillRect(this.x*size,(y -  0.25 - (i)*(4-i)*0.01625)*size-4, size,4);
        Game.gc.setFill(Color.GREEN);
        Game.gc.fillRect(this.x*size,(y -  0.25 - (i)*(4-i)*0.01625)*size-4, hp/hpmax*size,4);
    }
    @Override
